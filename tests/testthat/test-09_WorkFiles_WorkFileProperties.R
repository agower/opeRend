...
withr::defer(unlink(testUploadFile))
withr::defer(unlink(testDownloadFile), priority = "last")
  ...
  "addWorkFile correctly handles arguments",
  {
    ...
    expect_error(
      addWorkFile(
        file = testUploadFile,
        token = paste(
          sample(x = c(0:9,letters[1:6]), size = 63, replace = TRUE),
          collapse = ""
        )
      )
    )
...
test_that(
  "updateWorkFileProperties returns an operendWorkFileProperties object",
  {
    ...
    result <- updateWorkFileProperties(
      id = workFileId, originalName = "newName"
    )
    expect_s4_class(result, class = "operendWorkFileProperties")
  }
)
